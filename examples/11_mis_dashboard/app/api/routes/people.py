...
from app.api.deps import get_export_service, get_people_service
...
from app.services.people_service import PeopleService, UnknownEntity
...
async def advisor_list(
    ...
    return {"advisors": await people_service.advisor_list()}
...
async def branch_list(
    ...
    return {"branches": await people_service.branch_list()}
...
async def advisor_panel(
    ...
    try:
        return await people_service.advisor_panel(code)
...
async def cumulative(
    ...
    try:
        return await people_service.cumulative(scope, code, month)
    except UnknownEntity as exc:
        raise HTTPException(404, str(exc)) from None
...
async def advisor_panel_export(
    ...
    try:
        payload = await people_service.advisor_panel(code)
    except UnknownEntity as exc:
        raise HTTPException(404, str(exc)) from None
...
async def cumulative_export(
    ...
    if fmt not in MEDIA_TYPES:
        raise HTTPException(400, f"fmt must be one of {sorted(MEDIA_TYPES)}")
    try:
        payload = await people_service.cumulative(scope, code, month)
    except UnknownEntity as exc:
        raise HTTPException(404, str(exc)) from None
    except ValueError as exc:
        raise HTTPException(400, str(exc)) from None
