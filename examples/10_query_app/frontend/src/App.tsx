...
export function App(): ReactElement {
  ...
  const isRunning = Boolean(tab?.runningQueryId);
  const statusClass = state.status === 'Ready' ? 'is-ok' : /running/i.test(state.status) ? 'is-running' : 'is-error';
  return <div className="app-shell" onClick={() => setContext(null)}>
    <header className="topbar">
      <div className="brand"><span className="brand-mark" aria-hidden="true">◈</span><span className="brand-name">Netezza SQL Workspace</span><span className="brand-env">nzpy</span></div>
      <div className="run-group" role="group" aria-label="Query execution">
        <button className="primary run-main" onClick={() => void run('cursor')} disabled={isRunning} title="Run statement under cursor (Ctrl+Enter)">▶ Run</button>
        <button className="ghost" onClick={() => void run('selection')} disabled={isRunning} title="Run selected text">Run selection</button>
        <button className="ghost" onClick={() => void run('script')} disabled={isRunning} title="Run entire script">Run script</button>
        <button className="danger" onClick={cancel} disabled={!isRunning} title="Cancel running query">■ Cancel</button>
      </div>
      <span className="toolbar-spacer" />
      <span className="run-hint"><kbd>Ctrl</kbd>+<kbd>Enter</kbd> runs</span>
      <span className={`status-pill ${statusClass}`}><span className="status-dot" aria-hidden="true" /><span className="status-text">{state.status}</span></span>
    </header>
    <div className="sql-tabs" role="tablist" aria-label="SQL tabs">{state.tabs.map(item => <div key={item.id} role="tab" aria-selected={item.id === tab.id} className={`sql-tab ${item.id === tab.id ? 'active' : ''}`} onClick={() => dispatch({ type: 'tab.activate', id: item.id })} title={`${item.title}${item.dirty ? ' — unsaved changes' : ''} (double-click to rename)`}><span className="sql-tab-title" onDoubleClick={() => { const title = window.prompt('Tab name', item.title); if (title) dispatch({ type: 'tab.rename', id: item.id, title }); }}>{item.title}{item.dirty ? <i className="dirty-dot" aria-label="unsaved">•</i> : null}</span>{item.results.length > 0 && <span className="sql-tab-count">{item.results.length}</span>}<button onClick={event => { event.stopPropagation(); closeTab(item.id); }} title="Close tab" aria-label={`Close ${item.title}`}>×</button></div>)}<button className="new-tab" onClick={() => dispatch({ type: 'tab.add' })} title="New SQL tab">＋</button></div>
    <main className="workspace"><aside><SchemaTree refreshKey={schemaRefreshKey} onMenu={(node, event) => { event.preventDefault(); setContext({ x: event.clientX, y: event.clientY, items: schemaMenu(node, insert, openQuery, () => void refreshSchema(node.database, node.schema), () => void showDetail(node)) }); }} onSelect={node => { if (node.kind === 'object' || node.kind === 'column') { const name = node.kind === 'column' ? [node.database, node.schema, node.object_name, node.column_name || node.label] : [node.database, node.schema, node.object_name || node.label]; insert(name.filter(Boolean).join('.')); } }} onRefresh={() => void refreshSchema()} /></aside><section className="editor-results"><div className="editor-head"><span className="db-chip"><em>Database</em>{tab.database || 'Configured database'}</span><span className="db-chip"><em>Schema</em>{tab.schema || 'All schemas'}</span><span className="editor-hint">Click an object in Schema to insert its name</span><button className="ghost" onClick={() => insert('SELECT ')}>Insert SELECT</button></div><div className="editor"><Editor height="100%" language="sql" theme="vs-dark" path={tab.id} value={tab.sql} onChange={value => dispatch({ type: 'tab.sql', id: tab.id, sql: value || '' })} onMount={onMount} options={{ minimap: { enabled: false }, automaticLayout: true, fontSize: 13.5, wordWrap: 'on', bracketPairColorization: { enabled: true }, scrollBeyondLastLine: false, acceptSuggestionOnEnter: 'off', padding: { top: 10 }, fontFamily: "'JetBrains Mono', 'Cascadia Code', Consolas, monospace" }} /></div><ResultTabs results={tab.results} activeId={tab.activeResultId} onSelect={id => dispatch({ type: 'result.activate', tabId: tab.id, resultId: id })} onClose={id => dispatch({ type: 'result.close', tabId: tab.id, resultId: id })} /></section></main>
    {context && <ContextMenu x={context.x} y={context.y} items={context.items} onClose={() => setContext(null)} />}
    {detail && <div className="detail-overlay" onClick={() => setDetail(null)}><section className="detail-dialog" onClick={event => event.stopPropagation()}><div className="detail-head"><strong>{definition ? `${String(detail.object_type || 'Object')} definition` : 'Object details'}</strong><button onClick={() => setDetail(null)} title="Close" aria-label="Close details">×</button></div>{definition && <div className="definition-block"><div className="definition-toolbar"><span>Source definition</span><button onClick={() => void navigator.clipboard?.writeText(definition)}>Copy definition</button></div><pre className="source-code">{definition}</pre></div>}<pre className="detail-json">{JSON.stringify(detailMetadata, null, 2)}</pre></section></div>}
    {activeResult?.message && <div className="toast" role="status"><span className="toast-dot" aria-hidden="true" />{activeResult.message}</div>}
  </div>;
}
