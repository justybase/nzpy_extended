...
export const api = {
  ...
  detail: (node: SchemaNode) => json<Record<string, unknown>>(`/api/v1/schema/detail?${new URLSearchParams({ table: node.object_name || node.label, ...(node.schema ? { schema: node.schema } : {}), ...(node.database ? { database: node.database } : {}), ...(node.object_type ? { object_type: node.object_type } : {}) })}`),
